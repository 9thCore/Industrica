...
namespace Industrica.Utility
{
    public static class GameObjectUtil
    {
        ...
        public static void SetupConstructableBounds(this GameObject prefab, Vector3? offset = null)
        {
            offset ??= Vector3.zero;

            ConstructableBounds bounds = prefab.EnsureComponent<ConstructableBounds>();
            OrientedBounds.EncapsulateRenderers(
                prefab.transform.worldToLocalMatrix,
                prefab.GetComponentsInChildren<Renderer>().ToList(),
                out bounds.bounds.position,
                out bounds.bounds.extents);
